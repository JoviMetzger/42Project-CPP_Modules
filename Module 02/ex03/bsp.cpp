...
/* bsp();
 *	- Checks whether the given point is inside the triangle formed by three vertices.
 *	- To determine if a point is inside a triangle, 
 *	  we calculate the areas of the main triangle and its three sub-triangles.
 *	- If the point is inside the triangle, it should be 
 *	  on the same side (either all counterclockwise or all clockwise) of each of the three triangle edges.
 *	- We check if the areas PBC, APC, and ABP are all positive, 
 *	  indicating that the point is on the same side of each edge as the main triangle ABC.
 *
 *	Calculation:
 *		- Formula:
...
bool	bsp( Point const a, Point const b, Point const c, Point const point)
{
	...
	// Check if the point is inside the triangle based on the areas
